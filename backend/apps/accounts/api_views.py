...
class PersonalAuditLogListAPIView(APIView):
    ...
    def get(self, request):
        ...
        boxes_by_code = resolve_audit_box_references(
            logs,
            organization_id=organization.id,
        )
        measurements_by_id = resolve_audit_measurements(
            logs,
            organization_id=organization.id,
        )
        return Response(
            {
                "results": [
                    serialize_personal_audit_log(
                        log,
                        box=boxes_by_code.get(log.object_id),
                        measurement=measurements_by_id.get(
                            (log.metadata or {}).get("measurement_id")
...
class AdminAuditLogListAPIView(APIView):
    ...
    def get(self, request, pk=None):
        ...
        if include_options:
            ...
            family_counts = {
                row["business_family"]: row["count"]
                for row in logs_query.values("business_family").annotate(
                    count=Count("id")
                )
            }
            family_options = [
                {"key": family, "count": family_counts.get(family, 0)}
        ...
        if family_filter:
            selected_families = [
                family.strip()
                for family in family_filter.split(",")
                if family.strip()
            ]
            invalid_families = [
                family for family in selected_families if family not in AUDIT_FAMILIES
            ]
            if invalid_families:
                raise ValidationError({"family": "Famille d'action invalide."})
            logs_query = logs_query.filter(business_family__in=selected_families)
        ...
        related_action_counts = related_measurement_action_counts(
            logs,
            organization_id=organization_ids[0],
            valid_measurement_ids=measurements_by_id.keys(),
        )
        legacy_measurements_by_key = resolve_legacy_audit_measurements(
            logs,
            organization_id=organization_ids[0],
        )
        boxes_by_code = resolve_audit_box_references(
        ...
        if include_options:
            ...
            payload["family_options"] = family_options
    ...
    @staticmethod
    def _explicit_measurement_id(log):
    ...
    def _resolve_measurement(
        ...
        return legacy_measurements_by_key.get(legacy_measurement_lookup_key(log))
...
class AdminAuditLogLinkedAPIView(AdminAuditLogListAPIView):
    """Return one complete structured measurement audit chain."""
    ...
    def get(self, request, pk=None):
        ...
        if not organization_ids:
            raise PermissionDenied(
                "This account cannot view the audit log for the selected organization."
            )
        organization_id = organization_ids[0]
        root = get_object_or_404(
            impactful_audit_logs(organization_id=organization_id).select_related(
                "organization",
                "user",
                "edited_by",
            ),
        ...
        measurement_id = self._explicit_measurement_id(root)
        if (
            root.action not in {AuditLog.Action.ENTRY, AuditLog.Action.UPDATE}
            or root.business_family != "measurements"
            or measurement_id is None
        ):
            raise Http404
        ...
        logs = list(
            impactful_audit_logs(organization_id=organization_id)
            .filter(
                action__in=(AuditLog.Action.ENTRY, AuditLog.Action.UPDATE),
                business_family="measurements",
                metadata__measurement_id=measurement_id,
            )
            .select_related("organization", "user", "edited_by")
        ...
        related_action_count = max(len(logs) - 1, 0)
        return Response(
            {
                "results": [
                    self._serialize_log(
