...
const TrackingChart = lazy(async () => {
  ...
  return { default: function PreviewChart({ detail, language, t }: { detail: BoxDetail; language: Language; t: Translator }) {
    ...
    const initialOffset = useMemo(() => getLatestChartWindowOffset(detail.biological_measurements.map((measurement) => measurement.measured_on)), [detail]);
...
function BoxTrackingPreviewContent({ boxId, language, t }: { boxId: number; language: Language; t: Translator }) {
  ...
  return (
    <>
      {detail.status !== 'inactive' && detail.thermal_zone ? <p className="box-tracking-preview-location">{detail.thermal_zone.name}</p> : null}
      {detail.biological_measurements.length ? (
